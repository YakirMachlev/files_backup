...
{
    FILE *fp;
    char buffer[FRAGMENT_MAX_LENGTH + 5];
    char *name;
    uint8_t name_length;
    char *file_name;
    uint8_t file_name_length;
    ...
    {
        total_length = sprintf(buffer, "%c%c%hd%s", (uint8_t)DOWNLOAD_FILE_RESPONSE, is_last, content_len, content);
        send(fd, buffer, total_length, 0);
    }
    is_last = 1;
    total_length = sprintf(buffer, "%c%c%hd%s", (uint8_t)DOWNLOAD_FILE_RESPONSE, is_last, content_len, content);
    send(fd, buffer, total_length, 0);
